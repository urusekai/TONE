...
- `MV Mode` : 메인 플레이어에서 곡 재생과 함께 배경 영상을 감상할 수 있습니다.
...
- `백엔드 API 직접 구현` : 프론트엔드 화면에 연결되는 인증, 데일리 톤 추천, 플레이리스트, 캘린더, 팔레트 로그 기능을 PHP와 MySQL 기반 API로 직접 구현했습니다.
- `소셜 로그인 처리` : 카카오, 구글, 네이버 OAuth 콜백을 연동하고 세션 기반 로그인 흐름으로 연결했습니다.
- `배포 자동화` : GitHub Actions를 통해 `main` 머지 시 프론트 빌드, 백엔드 의존성 설치, 서버 배포가 자동으로 이루어지도록 구성했습니다.
- `협업 흐름 관리` : 브랜치 전략, 커밋 컨벤션, Pull Request 템플릿을 기반으로 작업 이력과 배포 단위를 관리했습니다.
- `미디어 분리 운영` : Cloudflare R2 공개 URL을 조합해 커버, 음원, 영상 리소스를 앱 서버와 분리해 관리했습니다.
