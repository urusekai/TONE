...
try {
    ...
    // LLM에 넘길 후보 8개를 서버에서 먼저 추린다.
    $candidateRows = pickDailySpectrumCandidates($playlistRows, $userAnswers, 8);
...
function pickDailySpectrumCandidates(array $playlistRows, array $userAnswers, int $limit = 8): array
{
    ...
    foreach ($playlistRows as $row) {
        ...
        $paceScore = scoreScaleMatch(
            (string) $userAnswers['day_pace'],
            (string) $row['day_pace'],
            ['slow', 'steady', 'fast']
        );
    ...
    usort(
        ...
        static function (array $left, array $right): int {
            ...
            if ($left['exact_matches'] !== $right['exact_matches']) {
                return $right['exact_matches'] <=> $left['exact_matches'];
            }
